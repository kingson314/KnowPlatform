...
<%@ page import="java.util.Map" %>
    <script type="text/javascript" language="javascript">
        function one(){
            document.getElementById("publications-top-menu").style.display="none";
            document.getElementById("orgunits-top-menu").style.display="none";
        };
        window.onload = one;
    </script>
...
<!--导航栏具体的关键字-->
       <div class="navbar-header">
         ...
         <a class="navbar-brand"  href="<%= request.getContextPath() %>/" style="display: none;"><img height="25"  src="<%= request.getContextPath() %>/image/dspace-logo-only.png" alt="DSpace logo" /></a>
       </div>
...
<!--四个写一起的选项卡-->
           <li id="<%= mlink.trim() %>-top-menu" class="hidden-xs hidden-sm <c:if test="${exploremlink == location}">active</c:if>"><a style="height:33px;" href="<%= request.getContextPath() %>/cris/explore/<%= mlink.trim() %>"><fmt:message key="${fmtkey}"/></a></li>
           <% } %>
